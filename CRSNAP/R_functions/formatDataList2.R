...
#' @export
formatDataList2 <- function(species, data_dir, predata_yrs){
    ...
    if(species=="CRSNAP"){
        ...
        if(predata_yrs != FALSE) tyears <- c((fishery_data$years[1]-predata_yrs):(fishery_data$years[1]-1),fishery_data$years)
        if(predata_yrs==FALSE) tyears <- fishery_data$years
        tyears_i <- 1:length(tyears)
