plot_LCdata <- function(lc_list, lc_years, gears, L50=NULL, S50=NULL, ylim=NULL, colors=c("darkorange", "purple"), legend=TRUE){
	...
	for(y in 1:length(lc_years)){
		if(length(lc_list)==1){
			...
			if(y %% dim[1]==1) axis(2, pretty(ylim)[-length(pretty(ylim))], cex.axis=2, las=2)
		}
		if(length(lc_list)>1){
			barplot(lc_list[[y]], col=col_vec[which(names(col_vec) %in% rownames(lc_list[[y]]))], border=NA, lwd=4, xaxs="i", yaxs="i", xlim=c(0,length(bins)), ylim=ylim, xaxt="n", yaxt="n", space=0)
			text(x=0.2*length(bins), y=0.9*max(ylim), lc_years[y], font=2, cex=2)
			box()
			if(is.null(L50)==FALSE) abline(v=which(bins>=ML50)[1], col="black", lwd=2, lty=2)
			if(is.null(S50)==FALSE) abline(v=which(bins>=S50)[1], col="gray", lwd=2, lty=2)
