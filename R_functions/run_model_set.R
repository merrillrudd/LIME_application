run_model_set <- function(dir, LC, lh, add_years=0, S_l_input=-1, C_t=NULL, I_t=NULL, F_up=5, est_sigma="log_sigma_R", fix_param=FALSE, param_adjust=FALSE, val_adjust=FALSE, data_avail="LC", rerun=TRUE){
	...
	input_data <- list("years"=years_m, "LF"=LC, "C_t"=C_t, I_t=I_t)
		...
		if(file.exists(file.path(dir, "Report.rds"))==FALSE | rerun==TRUE){
			res <- run_LIME(modpath=dir, write=TRUE, lh=plist, input_data=input_data, est_sigma=est_sigma, data_avail=data_avail, LFdist=1, simulation=FALSE, rewrite=TRUE, S_l_input=S_l_input, F_up=F_up, fix_param=fix_param, param_adjust=param_adjust, val_adjust=val_adjust)
		}
