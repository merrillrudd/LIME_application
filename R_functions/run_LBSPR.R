...
run_LBSPR <- function(modpath, lh, input_data=NULL, itervec=NULL, species=NULL, rewrite=TRUE, simulation=TRUE){
    ...
    for(iter in 1:length(itervec)){
        ...
        if(is.null(modpath)==FALSE & rewrite==FALSE) if(any(grepl("LBSPR_results", list.files(path=iterpath)))) next
      if(is.null(modpath)==FALSE & rewrite==TRUE){
        if(any(grepl("non_convergence", list.files(path=iterpath)))) unlink(file.path(iterpath, "non_convergence.txt"), TRUE)
      }

    if(is.null(modpath)==FALSE & simulation==TRUE){
    ...
    Nyears_comp <- nrow(inits$LF)
        ...
        if(isS4(lbspr_res)==FALSE){
          if(is.null(modpath)==FALSE) saveRDS("non_convergence", file.path(iterpath, "non_convergence.txt"))
          if(is.null(modpath)){
            LBSPR_outs <- NA
          }
        }
    ...
    if(is.null(modpath)) return(LBSPR_outs)
